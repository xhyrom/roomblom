...
class Queue : Command("queue", "Show the queue") {
        ...
        when (action) {
            "next" -> {
                ...
                event.editMessage(
                    ...
                    Button.primary("queue-next-${pageId}", "Next")
                        .withDisabled(getQueuePage(queue.toList(), pageId + 1).isEmpty())
                ).queue()
            }
    ...
    private fun getQueuePage(queue: List<Track>, page: Int): List<Track> {
        val start = page * 10
        val end = start + 10
    ...
    private fun buildEmbed(queue: List<Track>, page: Int, fullQueue: List<Track>): MessageEmbed {
